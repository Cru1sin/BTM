...
class MatDataLoader:
    ...
    def _parser_mat_data(self, battery_i_mat):
        ...
        for i in range(1, battery_i_mat.shape[1]):
            ...
            data_dict = {
                'charge_time': cycle_i_data['relative_time_min'][0], # 时间数据
                'current': cycle_i_data['current_A'][0], # 电流数据
                'voltage': cycle_i_data['voltage_V'][0], # 电压数据
                'temperature': cycle_i_data['temperature_C'][0] # 温度数据
            }

            # 这里选择不进行LHS，因为mat文件已经整理好了，每循环都是128个数据
            #sampled_data = self._latin_hypercube_sampling(data_dict, self.sample_size)
            sampled_data = data_dict
            ...
            for j in range(len(sampled_data['charge_time'])):
                ...
                data_j = np.concatenate([
                    np.array([
                        i,                                  # 循环次数
                        relative_time,                      # 和上一采样点间隔的充电时间
                        sampled_data['charge_time'][j],     # 从开始充电到现在累计的充电时间
                        sampled_data['current'][j],         # 电流
                        sampled_data['voltage'][j],         # 电压
                        float(sampled_data['temperature'][j])  # 温度
                    ]),
                    current_stats           # 电流统计特征，这里不统计电压特征因为控制过程中电压是模型计算出来的而不是采样得出的
                ])
                cycle_i.append(data_j)

            # 计算容量损失
            capacity_loss = cycle_i_data['capacity'][0] - battery_i_mat[0,i-1]['capacity'][0] # 单位：Ah
        ...
        soh_loss = label / self.max_capacity * 100 # 容量损失百分比，这里乘以100是为了方便计算
...
if __name__ == '__main__':
    ...
    # 测试代码
    xjtu = XJTUdata(root='/Users/cruisin/Documents/BTM/SOH/data', args=args)
